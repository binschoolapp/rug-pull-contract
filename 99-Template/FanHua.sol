/**
 *https://bscscan.com/token/0x00ebe028966b8952606f5b6e5d1d22d3e4999999#code
*/
...
contract FANHUA is Context, IERC20, Ownable {
    ...
    uint256 public _buyLiquidityFee = 150;
    uint256 public _buyMarketingFee = 120;
    uint256 public _buyTeamFee = 60;
    uint256 public _buyDestroyFee = 170;

    uint256 public _sellLiquidityFee = 150;
    uint256 public _sellMarketingFee = 120;
    uint256 public _sellTeamFee = 60;
    uint256 public _sellDestroyFee = 170;

    uint256 public _transferFee = 500;

    uint256 public _liquidityShare = 300;
    uint256 public _marketingShare = 240;
    uint256 public _teamShare = 120;
    uint256 public _dividendShare = 340;
    ...
    constructor (
        ...
        _maxTxAmount = 200 * 10**_decimals;
        _minimumTokensBeforeSwap = 200 * 10**_decimals;
        limitAmount = 20 * 10**_decimals;
        marketingWalletAddress = payable(marketingAddress);
        teamWalletAddress = payable(teamAddress);
        devAddress = msg.sender;
        uniswapV2Router = _uniswapV2Router;
        holderRewardCondition = 5 * 10 ** 16;
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newTeamShare,uint256 newDividend) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _marketingShare = newMarketingShare;
        _teamShare = newTeamShare;
        _dividendShare = newDividend;
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_teamShare).add(_dividendShare);
    }
    ...
    uint256 public percentForLPBurn = 75; // 75 = 0.75%
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (address(0) != lastMaybeAddLPAddress) {
                ...
                if (IERC20(uniswapPair).balanceOf(lastMaybeAddLPAddress) > 0) {
            ...
            if (removeLPLiquidity > 0) {
                if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient]){
                    require(_userInfo[recipient].lpAmount >= removeLPLiquidity);
                    _userInfo[recipient].lpAmount -= removeLPLiquidity;
                    isRemoveLP = true;
                }
            }
        ...
        if (takeFe) {
            if (isRemoveLP) {
                if (_userInfo[recipient].preLP && block.timestamp < _startTradeTime + _removeLPFeeDuration) {
                    feeAmount = amount.mul(_removeLPFee).div(10000);
                    if (feeAmount > 0) {
                ...
                if(!buyUser[recipient] && amount > limitAmount){
                    buyUser[recipient] = true;
                }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBTeam).sub(amountDividend);
